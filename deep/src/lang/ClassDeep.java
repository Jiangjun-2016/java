...
/**
 * 博客引用：http://blog.csdn.net/a327369238/article/details/52577040
 * 源码解读
 */
/**
 * Java基本类之一，反射机制的基础
...
//	private static final int ANNOTATION = 8192;//注释类型
//	private static final int ENUM = 16384;//枚举类型
//	private static final int SYNTHETIC = 4096;//合成类型
/**
 * synthetic总的来说，是由编译器引入的字段、方法、类或其他结构，主要用于JVM内部使用，为了遵循某些规范而作的一些小技巧从而绕过这些规范，有点作弊的感觉，只不过是由编译器光明正大的，人为是没有权限的
 */
/**
 * 有synthetic标记的field和method是class内部使用的，正常的源代码里不会出现synthetic field.
 */
/**
 * 一般有内部类的话，会有合成类型——synthetic
 */

//	private transient volatile Constructor<T> cachedConstructor;//缓存上面方法已获取的公有构造方法，供下次使用
//	private transient volatile Class<?> newInstanceCallerCache;//缓存调用本方法的最初对象的类Class对象，供安全检查使用
...
//
//	private Class(ClassLoader var1) {//唯一私有构造方法，说明Class不可由用户构造实例
//		this.classLoader = var1;
//	}
//
//	public String toString() {//这里重写toString，只区别了接口和Class
...
//	}
//
//	public native boolean isInstance(Object var1);//判断obj对象是否是该Class的实例
//
//	public native boolean isAssignableFrom(Class<?> var1);//判断cls是否是调用者同一类型或其子类类型
//
//	public native boolean isInterface(); //判断该Class是否是接口类型
//
//	public native boolean isArray();//判断该Class是否是数组类型
//
//	public native boolean isPrimitive();//判断该Class是否是基本类型，（八大基本类型+一特殊基本类型）除了8大基本类型，还有void
//
//	public boolean isAnnotation() {//判断该Class是否是注释类型
//		return (this.getModifiers() & 8192) != 0;
//	}
//
//	public boolean isSynthetic() {//判断该Class是否是合成类型
...
//		if(var1 == null) {
//			this.name = var1 = this.getName0();//通过本地方法获取全路径类名
//		}
//
//		return var1;
//	}
...
//
//	@CallerSensitive
//	public ClassLoader getClassLoader() {//获取加载该类的类加载器
...
//	}
//
//	public TypeVariable<Class<T>>[] getTypeParameters() { //返回该类中变量字段的类型变量数组，按声明顺序排序，
...
//	}
//
//	public native Class<? super T> getSuperclass();//获得该类的直接父类的Class对象，如果该类是接口，则返回null
//
//	public Type getGenericSuperclass() {//返回带参数化类型的直接父类的类型
//		ClassRepository var1 = this.getGenericInfo();
//		return (Type)(var1 == null?this.getSuperclass():(this.isInterface()?null:var1.getSuperclass()));
//	}
//
//	public Package getPackage() {//获取该类的包路径
//		return Package.getPackage(this);
//	}
//
//	public Class<?>[] getInterfaces() {//获取该类直接实现的所有接口
...
//
//	public Type[] getGenericInterfaces() {//获取所有接口，同上面的不同之处在于，若超接口是参数化类型（泛型）则返回的是其实际类型
//		ClassRepository var1 = this.getGenericInfo();
...
//		return var1 == null?null:new Class.EnclosingMethodInfo(var1, null);
//	}
//
...
//		} else {
//			Object var2 = var1.assertionLock;
...
//			reflectionFactory = (ReflectionFactory)AccessController.doPrivileged(new GetReflectionFactoryAction());
//		}
//
...
//
//			try {
//				final Method var1 = this.getMethod("values", new Class[0]);
//				AccessController.doPrivileged(new PrivilegedAction() {
//					public Void run() {
...
//					}
//				});
//				Object[] var2 = (Object[])((Object[])var1.invoke((Object)null, new Object[0]));
...
//		return super.isAnnotationPresent(var1);
//	}
//
